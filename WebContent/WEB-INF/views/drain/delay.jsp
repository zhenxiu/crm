...
<html>
  ...
  <div class="button_bar">
	...
	<button class="common_button" onclick="window.location.href='${ctx}/drain/confirm?drainId=${drain.id }'">确认流失</button>
	<button class="common_button" onclick="document.forms[0].submit();">保存</button>
  </div>
	
  <body class="main">
	  <form action="${ctx}/drain/delay" method="post">
		  	<input type="hidden" name="drainId" value="${drain.id }"/>
			<table class="query_form_table" border="0" cellPadding="3" cellSpacing="0">
				...
				<c:if test="${not empty delays}">	
					<c:forEach items="${delays }" var="delay" varStatus="index">
						<tr>
							<th>暂缓措施-${index.count}</th>
